# purpose of this code: run in vitro TCE model (ModelingToolkit version) for glofit example
...
include(@projectroot("invitro-MTK/invitro_tce.jl"));
...
@mtkbuild tce = invitro();
prob0 = ODEProblem(tce, [], tspan_s, [tce.TCE => test_dose]);
sol0 = solve(prob0, alg=AutoTsit5(Rosenbrock23()), saveat = s_per_hr, abstol=1e-6, reltol=1e-3);

prob_glo = remake(prob0, p = Dict([tce.Tumor_cells_init => Tumor_cells_init, tce.total_T_cells_init => T_cells_init])); 
...
prob_glo2 = remake(prob0, p = Dict([tce.KD_CD3 => update_KD_CD3, tce.KD_TAA => update_KD_TAA, tce.Thalf_TAA => update_Thalf_TAA, tce.tt50_kill => try_tt50_kill, tce.emax_apop => try_emax_apop, tce.Tumor_cells_init => Tumor_cells_init, tce.total_T_cells_init => T_cells_init, tce.tt50_t_activate => try_tt50_t_activate, tce.emax_t_activate => try_emax_t_activate, tce.n_act => try_n_act])); 

output_0 = varies_readout(tcb_conc2*1E-3, prob_glo2);
output_2 = varies_readout(tcb_conc2*1E-3, remake(prob_glo2, p = Dict([tce.lambda_taa_avidity => 1E2])));
output_4 = varies_readout(tcb_conc2*1E-3, remake(prob_glo2, p = Dict([tce.lambda_taa_avidity => 1E4])));
output_6 = varies_readout(tcb_conc2*1E-3, remake(prob_glo2, p = Dict([tce.lambda_taa_avidity => 1E6])));
