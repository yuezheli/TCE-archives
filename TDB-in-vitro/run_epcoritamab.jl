...
function cytotox(p_update, obs = RI1_epco; prob = prob0, runOpt = true)
    ...
    newparam = Dict([tce.lambda => exp(p_update[1]), 
                     ...
                     tce.TAA_per_cell => exp(p_update[4]), 
...
p0 = log.([getdefault(tce.lambda), getdefault(tce.Thalf_TAA), getdefault(tce.k_apop), getdefault(tce.TAA_per_cell), getdefault(tce.ec50_kill)])
...
opt_prob = OptimizationProblem(cytotox, p0, daudi_epco);
solopt = solve(opt_prob, NLopt.LN_NELDERMEAD());
jldsave("data/param/optim_epcoritamab.jld2"; lambda = exp(solopt.u[1]), thalf_taa = exp(solopt.u[2]), k_apop = exp(solopt.u[3]), TAA_per_cell = exp(solopt.u[4]), ec50_kill = exp(solopt.u[5]));
...
p_bell_curve = plot(xlabel = "epcoritamab conc (ng/mL)", ylabel = "Cytotoxicity (%)", xaxis = :log10, legend = :bottomright, dpi = 300);
...
# save figure 
savefig(p_bell_curve, "figure/epcoritamab-in-vitro.png");
